...
class FeatureCategoricalFoam(Feature):
    """Categorical feature to FOAM file

    Alias for Feature with set.Categorical first sub action and get.file.markup.Foam last
    ...
    def __init__(self, choices, route='.~~', template=None,
        ...
        f = Foam(template=template, pattern=pattern, input_path=input_path,
                 output_path=output_path, remove_input=remove_input)
        kwargs.setdefault('sub_actions', []).append(f)
